<?php
    ...
    if(isset($_GET['read'])){
        ...
        $sqlQuery = "UPDATE `guardian_notifications` SET `status`='Read' WHERE `notfId`='$notificationId'";
        if(!$conn->query($sqlQuery)){
            echo $conn->error;
        }
        else{
            header('Location:guardCheckNotification.php');
        }
    }
        ...
        if($queryRun = $conn->query($sqlQuery)){
            ...
            if($dataRow['filled'] < $dataRow['capacity']){
                ...
                if($conn->query($sqlQuery1)){
                    ...
                    $sqlQuery3 = "UPDATE `guardian_notifications` SET `status`='Read' WHERE `notfId`='$notificationId'";
...
<html>
<head>
	<title> Guardian Notifications - OMS</title>
    ...
    <h1 style="color:white;"><?php echo $errorMessage; ?></h1>
        <table class="apptable">
            ...
            <?php
                if($conn)
                {
                    $sqlQuery = "SELECT * FROM `guardian_notifications` WHERE `id`='$accountId' and `status`='Unread'";
                    if($queryRun = $conn->query($sqlQuery)){
                        while($dataRow = mysqli_fetch_array($queryRun)){
                            ...
                            echo "<td><a style = 'color:white; font-weight:700; border:4px;' href='guardCheckNotification.php?notfId={$notfId}&read=True'>Mark As Read</a></td>";
                            if($dataRow['type'] == 'event'){
                                $eventId = $parsedMessage['eventid'];
                                echo"<td><a style = 'color:white; font-weight:700; border:4px;' href='guardCheckNotification.php?notfId={$notfId}&seatConfirm=True&eventId={$eventId}'>Confirm Seat</a></td>";
                            }
                            echo "</tr>";
                        }
                    }
        ...
        <table class="apptable">
            ...
            <?php
                if($conn)
                {
                    $sqlQuery = "SELECT * FROM `guardian_notifications` WHERE `id`='$accountId' and `status`='Read'";
                    if($queryRun = $conn->query($sqlQuery)){
                        while($dataRow = mysqli_fetch_array($queryRun)){
                            $message = parse_str($dataRow['message'], $parsedMessage);
                            $messageFinal = $parsedMessage['Message'];
