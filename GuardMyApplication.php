...
<html>
<head>
	<title>Guardian My Applications - OMS</title>
...
<body>
    ...
    <div class="menu">
		...
		<a href="GuardMyApplication.php"><img src="styling/icons/045-monitor.png" class="menuicons">My Applications</a>
		<a href="SponGuardRegisterhtml.php"><img src="styling/icons/approve.png" class="menuicons"><span>Apply as Guardian</span></a>
		<a href="#"><img src="styling/icons/036-calendar.png" class="menuicons"><span>Notifications</span></a>
    </div>
    <div class="tablebox">
        <table class="apptable">
            <caption> My Guardian Applications - Submitted</caption>
            <tr>
            ...
            <?php
                if($conn)
                {
                    $sqlQuery = "SELECT * FROM `guardian` WHERE `id` = '$accountId'";
                    $execute = $conn->query($sqlQuery);
                    if($execute)
                    {
                        $data = mysqli_fetch_array($execute);
                        $cnic = $data['cnic'];
                        $sqlQuery2 = "SELECT * FROM `guardian_application` WHERE `cnic` = '$cnic'";  
                        $execute2 = $conn->query($sqlQuery2);
                        while($dataRow = mysqli_fetch_array($execute2)){
                            $appId = $dataRow['guardAppID'];
                            $email = $dataRow['email'];
                            $cnic = $dataRow['cnic'];
